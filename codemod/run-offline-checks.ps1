...
try {
    ...
    if (-not $SkipPython) {
        ...
        Invoke-Step "Python tooling smoke tests" {
            ...
            Invoke-Native 'python' @('tools\test_dcl_item_sidecar.py')
            Invoke-Native 'python' @('tools\test_dcl_ai_scoring_boundary.py')
            ...
            Invoke-Native 'python' @('tools\test_dcl_canonical_admission_probe_readiness.py')
            Invoke-Native 'python' @('tools\test_dcl_canonical_admission_template_live.py')
            Invoke-Native 'python' @('tools\test_collect_dcl_canonical_admission_live_log.py')
            Invoke-Native 'python' @('tools\test_prepare_canonical_admission_live.py')
            Invoke-Native 'python' @('tools\test_validate_dcl_live_proof_sequence.py')
            ...
            Invoke-Native 'python' @('tools\test_dcl_weapon_line_of_fire.py')
            ...
            Invoke-Native 'python' @('tools\validate_dcl_runtime_data_pair.py', 'work\1784683300-dcl-active-integrated-runtime-data-pair.json')
            Invoke-Native 'python' @('tools\validate_dcl_live_regression_matrix.py')
            Invoke-Native 'python' @('tools\install_dcl_live_bundle.py')
            ...
            # Historical unified sentinels v2/clean-v1 depended on the retired
            # Immobilize/Disable duration-transfer experiment. The current DCL
            # status policy marks those shared producers design-open, so these
            # artifacts remain journal evidence rather than active offline gates.
            Invoke-Native 'python' @('tools\build_dcl_clean_regression_matrix.py', '--check-only')
            Invoke-Native 'python' @('tools\validate_dcl_live_regression_matrix.py', 'work\1784470893-dcl-clean-v1-live-regression-matrix.json', '--skip-runtime-pair-validation')
            ...
            Invoke-Native 'python' @('tools\analyze_dcl_weapon_line_of_fire.py', '--check-only')
            ...
            Invoke-Native 'python' @('tools\analyze_dcl_dark_knight_formulas.py', '--check-only')
            Invoke-Native 'python' @('tools\analyze_dcl_drain_transactions.py', '--check-only')
            Invoke-Native 'python' @('tools\analyze_dcl_dragon_gift.py', '--check-only')
            Invoke-Native 'python' @('tools\analyze_dcl_self_destruct.py', '--check-only')
            Invoke-Native 'python' @('tools\analyze_dcl_status_authority.py', '--check-only')

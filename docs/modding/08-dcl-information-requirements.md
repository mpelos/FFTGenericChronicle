...
| Reaction triggering | DCL reactions depend on authored taxonomy and exact action context. | **Proven/Strong:** the four real-code Brave-roll call sites expose the exact evaluated reaction id and accept an authored chance; a cached DCL miss suppresses their chance before the native roll. VM-owned avoidance receives the exact id from `orderRecord+2` and can consume a scoped virtual Brave value restored at the sole calculation exit. Reaction set bitfield `unit+0x94..0x97` and dispatcher `0x30B4EC` are Strong static. Pass-2 commit cardinality and state-`0x2C` per-strike delivery are Proven live. The accepted post-selector boundary `0x2063BD` is Proven live to expose the complete native order before actor construction and to distinguish selected unit-table indices from later actor indices. A fail-closed order replacement/source-retarget controller owns that boundary. The offline-tested synthetic-Reaction transaction adds an exact equipped-carrier successful-result reservation, configurable courage/caution/neutral roll, dynamic empty-slot producer, guarded accepted-order transformation, and producer-owned once-per-attacker-action pass-2 commit without embedding a job or effect. The composed vertical remains live-gated, and representative native, VM-owned, and synthetic trigger families still need integrated live regression. See `04` and `06`. |

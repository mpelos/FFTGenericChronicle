...
| True action id / ability id | Damage type, spell power, status category, target mode, element, and special behavior depend on the exact action. | **Mixed (LT1 2026-07-02)**: pending record `unit+0x1A1/+0x1A2` **Proven live** (Fire = `0x10`, Blind = `0xEA`, enemy skill = `0x118` — classic id space confirmed) at CONFIRM time; `dword[0x14186AFF4]` (caster index) **Proven live** across player+enemy actions. `word[0x14186AFF0]` **Refuted as current-action id** (stayed 0 through previews) — it holds the **reaction ability id under evaluation** (445/451 at parry/Mana-Shield moments). Preview-time id path = LT3 hook `computeActionResult 0x309A44` (`rcx+2`), which provably runs at preview. See `work/dcl-action-id-candidates.md`, `work/lt1-mega-probe-plan.md`. |
...
| Status infliction | DCL status contests must replace native status odds. | **Updated by LT2 (2026-07-02)**: `g_7B07AC` observably correlates with the staged status outcome (0 on the immune 0% case, 3854 on the 100% case) but the **data-poke lever is Refuted** — the engine rewrites it at compute time (poked 0 → engine wrote 3854 back → Blind landed at 100%). Two live-proven controls remain: (a) **immunity-bit input-control** (`+0x5C..0x60`, forecast 0% + miss — proven LT2) for suppression; (b) the compute-time hook `0x30662C` (fn `0x3065F0`, roll `0x306636`) for arbitrary odds — LT3. See `work/dcl-magic-status-reaction-candidates.md`, `work/lt1-mega-probe-plan.md`. |

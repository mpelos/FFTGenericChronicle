...
def ability_flags(row: dict[str, str]) -> str:
    flags = []
...
def ability_doc(rows: list[dict[str, str]]) -> str:
    ...
    for row, command, owner, tags in row_data:
        ...
        lines.append(
            f"| {ability_id} | `{name}` | {command} | {owner} | {row['JP']} | {ability_flags(row)} | "
            f"{summary_from_tags(tags)} | {tag_list(tags)} |"
        )
    lines += [
        "",
        "## Design Use",
...
def command_effect_map_doc(rows: list[dict[str, str]]) -> str:
    row_data = ability_row_data(rows)
    by_command: dict[str, list[tuple[dict[str, str], str, str, list[str]]]] = defaultdict(list)
    for item in row_data:
        _, command, _, _ = item
        by_command[command].append(item)
    ...
    for command, rows_for_command in by_command.items():
        owner = rows_for_command[0][2]
        tag_counts = Counter(tag for _, _, _, tags in rows_for_command for tag in tags)
        tag_mix = ", ".join(f"`{tag}` x{count}" for tag, count in sorted(tag_counts.items()))
        lines.append(f"| {command} | {owner} | {len(rows_for_command)} | {tag_mix} |")
    ...
    for command, rows_for_command in by_command.items():
        ...
        lines += [
            f"### {command}",
            "",
            f"- Vanilla owner: {owner}",
            f"- Records: {len(rows_for_command)}",
            f"- Tag mix: {tag_mix}",
            "",
            "| Id | Ability | JP | Local flags | Effect summary | Tags |",
            "| ---: | --- | ---: | --- | --- | --- |",
        ]
        for row, _, _, tags in rows_for_command:
            ability_id = int(row["Id"])
            name = normalize_name(row["Name"])
            lines.append(
                f"| {ability_id} | `{name}` | {row['JP']} | {ability_flags(row)} | "
                f"{summary_from_tags(tags)} | {tag_list(tags)} |"
            )
        lines.append("")
...
def effect_crosswalk_doc(rows: list[dict[str, str]]) -> str:
    row_data = ability_row_data(rows)
    by_tag: dict[str, list[tuple[dict[str, str], str, str, list[str]]]] = defaultdict(list)
    for item in row_data:
        for tag in item[3]:
    ...
    for tag, desc in sorted(TAG_DESCRIPTIONS.items()):
        lines.append(f"| `{tag}` | {len(by_tag.get(tag, []))} | {TAG_DESIGN_USE[tag]} |")
    ...
    for tag, desc in sorted(TAG_DESCRIPTIONS.items()):
        rows_for_tag = by_tag.get(tag, [])
        lines += [
            f"### `{tag}`",
            "",
            f"- Meaning: {desc}",
            f"- Design use: {TAG_DESIGN_USE[tag]}",
            f"- Records: {len(rows_for_tag)}",
            "",
            "| Id | Ability | Command bucket | Vanilla owner | JP | Local flags | Other tags |",
            "| ---: | --- | --- | --- | ---: | --- | --- |",
        ]
        for row, command, owner, tags in rows_for_tag:
            ability_id = int(row["Id"])
            name = normalize_name(row["Name"])
            others = [other for other in tags if other != tag]
            other_text = tag_list(others) if others else "-"
            lines.append(
                f"| {ability_id} | `{name}` | {command} | {owner} | {row['JP']} | "
                f"{ability_flags(row)} | {other_text} |"
            )
        lines.append("")
...
def main() -> int:
    ...
    (DOCS / "fft-vanilla-command-skillset-effect-map.md").write_text(
        command_effect_map_doc(ability_rows), encoding="utf-8"
    )
    (DOCS / "fft-vanilla-ability-effect-tag-crosswalk.md").write_text(
        effect_crosswalk_doc(ability_rows), encoding="utf-8"
    )
    ...
    print(f"wrote {DOCS / 'fft-vanilla-command-skillset-effect-map.md'}")
    print(f"wrote {DOCS / 'fft-vanilla-ability-effect-tag-crosswalk.md'}")
    return 0

...
Row manifest: `work/1783985050-dcl-status-policy.csv`.
...
| lifecycle-mechanism-required | 14 |
| surface-ready | 195 |
...
| Dead | 13 | add-harmful, remove-ko | lifecycle | explicit-rule, none | lifecycle-mechanism-required |
...
- `Dead`, `Undead`, and `Crystal` live in status byte 0. Generic `DclStatusRule` whitelists only proven-safe Undead `0x10`; KO and Crystal remain dedicated lifecycle gates.
